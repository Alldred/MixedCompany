...
for stem,title in [('MixedCompany','MixedCompany / standard'),('MixedCompanyMono','MixedCompany Mono')]:
    canvas=Image.new('RGB',(1680,1440),'#f5f0e4');draw=ImageDraw.Draw(canvas)
    draw.text((60,40),title+' / programming ligatures',font=ImageFont.truetype(ui,38),fill='#252621')
    draw.text((60,103),'Each sequence has its own design. Shaped results at 110 px and 40 px.',font=ImageFont.truetype(ui,23),fill='#78786b')
    for i,sequence in enumerate(sequences):
        x=60+(i%4)*400;y=180+(i//4)*300
        draw.rectangle((x,y,x+385,y+280),outline='#c9c1b2',width=2)
        ...
        subprocess.run(['hb-view',str(ROOT/'outputs'/stem/(stem+'-Regular.ttf')), '--text='+sequence,
            '--font-size=40','--background=f5f0e4','--foreground=252621','--margin=0',
            '--output-file='+str(out)],check=True)
        with Image.open(out) as rendered:
            canvas.paste(rendered.convert('RGB'),(x+(385-rendered.width)//2,y+210))
